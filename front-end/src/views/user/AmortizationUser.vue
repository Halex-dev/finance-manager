<template>
    <CRow>
        ...
        <CCol :md="12">
            <CCard class="mb-4">
                <CCardBody>
                    <CCardTitle>Cost Amortization</CCardTitle>
                    <CContainer class="pt-2">
                      <CRow class="align-items-end">
                          ...
                          <CCol xs="2" class="d-flex justify-content-end align-items-end">
                              <span class="mx-1" @click="fetchAmorByDate()">
    ...
    <CModal size="lg" alignment="center" backdrop="static" :visible="ModalAdd" @close="CloseModal()">
        <CModalHeader>
        <CModalTitle>Amortization Cost</CModalTitle>
        </CModalHeader>
        <CModalBody>
        <CForm class="row g-3">
            <CCol md="12">
            ...
            <CCol md="6">
              <CFormLabel for="inputDate">Start date</CFormLabel>
  ...
  mounted() {
        ...
        // Calcolare il mese di endDate
        let endMonth = currentDate.getMonth() + 4;

        // Aggiustare l'anno se il mese supera dicembre
        if (endMonth > 11) {
            endMonth = endMonth % 12; // Restituisce il resto della divisione per 12
            currentDate.setFullYear(currentDate.getFullYear() + 1); // Aumenta l'anno di 1
        }

        // Impostare endDate sulla nuova data
        this.endDate = new Date(currentDate.getFullYear(), endMonth, 1); //3 mesi in più a partire da oggi

        this.fetchAmorByDate();
  ...
  methods: {
    ...
    async fetchAmor() { //Function to fetch the table
      try {
          const responseCosts = await axios.get('http://localhost:3000/api/amortizations/');
          this.rows = responseCosts.data;
      } catch (error) {
          console.error('Error fetching costs:', error);
      }
    },
    async fetchAmorByDate(){
      try {
          const responseCosts = await axios.get('http://localhost:3000/api/amortizations', {
              params: {
          ...
          this.rows = responseCosts.data;
          console.log(responseCosts.data);
      } catch (error) {
          console.error('Error fetching costs:', error);
      }
    },
    async addCost() { //Add a row in the database of the server 
      ...
      try {
          ...
          this.fetchAmorByDate(); // Update the table
      } catch (error) {
